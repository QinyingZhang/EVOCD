...
class v8ClassificationLoss:
    ...
    def __init__(self, weight=None):
        """Initialize the v8ClassificationLoss class with optional class weights."""
        self.weight = weight

    def __call__(self, preds, batch):
        """Compute the classification loss between predictions and true labels."""
        preds = preds[1] if isinstance(preds, (list, tuple)) else preds
        self.weight = self.weight.to(preds.device) if self.weight is not None else None
        loss = F.cross_entropy(preds, batch["cls"], weight=self.weight, reduction="mean")
