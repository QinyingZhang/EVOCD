"""
损失函数消融实验4测试：仅Focal Loss（无类别权重，无过采样，无数据增强）
加载 strategy_4_focal_loss_only 的最佳模型并在测试集上评估性能。
"""
import argparse
import json
import sys
...
def plot_confusion_matrix(cm: np.ndarray, output_path: Path, num_classes: int) -> None:
    ...
    plt.title("Confusion Matrix - Strategy 4 (Focal Loss Only)")
...
if __name__ == "__main__":
    print("=" * 70)
    print("测试策略4：仅Focal Loss（无类别权重，无过采样，无数据增强）")
    print("=" * 70)
    ...
    TRAIN_RUN_DIR = (
        ...
        / "train"
        / "runs"
        / "strategy_4_focal_loss_only"
        / "train_01"
    )

    parser = argparse.ArgumentParser(description="测试策略4：仅Focal Loss")
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(SCRIPT_DIR / "runs" / "strategy_4_focal_loss_only"),
    )
