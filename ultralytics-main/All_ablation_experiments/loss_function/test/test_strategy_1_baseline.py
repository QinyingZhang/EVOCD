"""
损失函数消融实验1测试：基线（CrossEntropy，无过采样，无数据增强）
根据训练得到的 best_model.pth 在测试集上评估性能
"""
import argparse
import sys
...
# -----------------------------------------------------------------------------
# 环境准备：把 hier_mult 添加到路径，便于复用训练阶段的数据/模型定义
# -----------------------------------------------------------------------------
...
# 训练时用到的模块
from archived.model_concat_fusion import (  # type: ignore
    HierarchicalFloodClassifier_ConcatFusion,
)
...
def evaluate_model(
    ...
    precision = precision_score(
        all_labels_np, all_preds_np, average="weighted", zero_division=ZERO_DIVISION
    )
    recall = recall_score(
        all_labels_np, all_preds_np, average="weighted", zero_division=ZERO_DIVISION
    )
    ...
    macro_precision = precision_score(
        all_labels_np, all_preds_np, average="macro", zero_division=ZERO_DIVISION
    )
    ...
    for class_idx in range(num_classes):
        ...
        if class_total > 0:
            ...
            per_class_recall.append(float(ZERO_DIVISION))
...
def plot_confusion_matrix(cm: np.ndarray, output_path: Path, num_classes: int) -> None:
    ...
    plt.title("Confusion Matrix - Strategy 1 (Baseline)")
...
if __name__ == "__main__":
    print("=" * 70)
    print("Test Strategy 1: Baseline (CrossEntropy, no oversampling, no augmentation)")
    print("=" * 70)
    ...
    parser = argparse.ArgumentParser(description="Test Strategy 1: Baseline configuration")
    ...
    parser.add_argument(
        "--model_path", type=str, default=str(TRAIN_RUN_DIR / "best_model.pth")
    )
    parser.add_argument(
        "--output_dir", type=str, default=str(SCRIPT_DIR / "strategy_1_baseline")
    )
    ...
    for class_idx, recall_value in enumerate(metrics["per_class_recall"]):
        print(f"  等级 {class_idx}: {recall_value:.4f}")
    ...
    plot_confusion_matrix(
        np.array(metrics["confusion_matrix"]), cm_path, NUM_FLOOD_GRADES
    )
