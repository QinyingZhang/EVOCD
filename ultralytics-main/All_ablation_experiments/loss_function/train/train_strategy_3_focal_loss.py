"""
损失函数消融实验3：Focal Loss（无过采样，无数据增强）
使用简单拼接融合模型
训练策略：完全分阶段训练（不冻结）
"""
import argparse
...
if __name__ == '__main__':
    print("=" * 70)
    print("训练策略消融实验3：Focal Loss")
    print("Focal Loss (γ=1.5) | 类别权重 | 无过采样 | 无数据增强")
    print("=" * 70)
    ...
    # 创建参数解析器
    parser = argparse.ArgumentParser(description='训练策略消融：Focal Loss')
    ...
    parser.add_argument('--epochs', type=int, default=100)  # 统一为100轮
    ...
    parser.add_argument('--use_focal_loss', type=bool, default=True)  # 开启Focal Loss
    parser.add_argument('--focal_gamma', type=float, default=1.5)
    
    # 训练策略
    parser.add_argument('--mixed_precision', type=bool, default=True)
    parser.add_argument('--patience', type=int, default=20)  # 统一为20
    ...
    parser.add_argument('--runs_dir', type=str, default=str(SCRIPT_DIR / 'runs' / 'strategy_3_focal_loss'))
    ...
    print("📋 配置: 完全分阶段训练（不冻结）| Focal Loss (γ=1.5) | 类别权重 | 无过采样 | 无数据增强")
