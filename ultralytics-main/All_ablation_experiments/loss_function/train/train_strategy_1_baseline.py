"""
损失函数消融实验1：基线（CrossEntropy，无过采样，无数据增强）
使用简单拼接融合模型
训练策略：完全分阶段训练（不冻结）
"""
import argparse
...
if __name__ == '__main__':
    print("=" * 70)
    print("训练策略消融实验1：基线配置")
    print("CrossEntropy | 无类别权重 | 无过采样 | 无数据增强")
    print("=" * 70)
    ...
    # 创建参数解析器
    parser = argparse.ArgumentParser(description='训练策略消融：基线')
    ...
    parser.add_argument('--resume', type=str, default=None, 
                       help='恢复训练的检查点路径，例如: runs/strategy_1_baseline/train_01/last_model.pth')
    
    # 其他
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--runs_dir', type=str, default=str(SCRIPT_DIR / 'runs' / 'strategy_1_baseline'))
    
    args = parser.parse_args()
    
    # 如果指定了resume路径，使用该路径；否则尝试自动检测
    resume_path = None
    if args.resume:
        resume_path = args.resume
        print(f"\n📂 使用指定的检查点: {resume_path}")
    else:
        # 尝试自动检测最新的检查点
        runs_dir = Path(args.runs_dir)
        if runs_dir.exists():
            # 查找所有训练目录
            train_dirs = sorted([d for d in runs_dir.iterdir() if d.is_dir() and d.name.startswith('train_')], 
            ...
            for train_dir in train_dirs:
                ...
                if last_model_path.exists():
                    resume_path = str(last_model_path)
                    print(f"\n🔍 自动检测到检查点: {resume_path}")
                    print(f"   将从该检查点恢复训练")
                    break
    
    # 创建训练器（使用原始的 Trainer 类）
    # 注意：先不设置resume，等模型替换后再加载
    args.resume = None  # 临时清空，稍后手动加载
    ...
    print("📋 配置: 完全分阶段训练（不冻结）| CrossEntropy | 无类别权重 | 无过采样 | 无数据增强")
