...
VEHI_SIDE_DIR = SCRIPT_DIR.parent
DATA_YAML = VEHI_SIDE_DIR / "side_d.yaml"
TEST_IMAGES_DIR = VEHI_SIDE_DIR / "dataset" / "images" / "test"
DEFAULT_PROJECT_DIR = SCRIPT_DIR / "runs"


def _build_parser(default_weights: Path, default_name: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate side-orientation YOLO detectors on the held-out test split."
    )
    parser.add_argument(
        "--weights",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory for YOLO evaluation outputs.",
    )
    parser.add_argument(
        "--name",
        type=str,
...
def run_side_test(default_weights: Path, default_name: str) -> None:
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    run_side_test(args.weights, args.name)
