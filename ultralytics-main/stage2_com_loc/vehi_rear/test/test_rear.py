...
VEHI_REAR_DIR = SCRIPT_DIR.parent
DATA_YAML = VEHI_REAR_DIR / "rear_d.yaml"
TEST_IMAGES_DIR = VEHI_REAR_DIR / "dataset" / "images" / "test"
DEFAULT_PROJECT_DIR = SCRIPT_DIR / "runs"


def _build_parser(default_weights: Path, default_name: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate rear-orientation YOLO detectors on the held-out test set."
    )
    parser.add_argument(
        "--weights",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory to store YOLO evaluation artifacts.",
    )
    parser.add_argument(
        "--name",
        type=str,
        default=default_name,
        help="Sub-folder name inside project dir.",
    )
    parser.add_argument(
        "--save_txt",
        action="store_true",
        help="Save predictions as YOLO txt files.",
    )
    parser.add_argument(
        "--save_conf",
        action="store_true",
    ...
    parser.add_argument(
        "--save_json",
        action="store_true",
        help="Export COCO-style JSON.",
    )
    parser.add_argument(
        "--plots",
        action="store_true",
        help="Save PR curve, confusion matrix, etc.",
    )
    return parser
...
def run_rear_test(default_weights: Path, default_name: str) -> None:
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    run_rear_test(args.weights, args.name)
