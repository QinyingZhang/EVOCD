...
VEHI_REAR_SIDE_DIR = SCRIPT_DIR.parent
DATA_YAML = VEHI_REAR_SIDE_DIR / "rear_side_d.yaml"
TEST_IMAGES_DIR = VEHI_REAR_SIDE_DIR / "dataset" / "images" / "test"
DEFAULT_PROJECT_DIR = SCRIPT_DIR / "runs"


def _build_parser(default_weights: Path, default_name: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate rear-side orientation detectors on the test split."
    )
    parser.add_argument(
        "--weights",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory to store YOLO evaluation outputs.",
    )
    parser.add_argument(
        "--name",
        type=str,
    ...
    parser.add_argument(
        "--save_json",
        action="store_true",
        help="Export COCO-style JSON results.",
    )
    parser.add_argument(
        "--plots",
        action="store_true",
...
def run_rear_side_test(default_weights: Path, default_name: str) -> None:
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    run_rear_side_test(args.weights, args.name)
